...
def generatePredictionFunctions():
    obverse = loadImages('grey', './Images/MSDs/')
    templates = loadImages('grey', './Images/MSDTemplates/')
    ...
    if save == "y":
        with open('MSD_Prediction_Function.txt', 'w') as writer:
            for coefficient in MSDp.coefficients:
                writer.write(str(coefficient) + '\n')

    obverse = loadImages('grey', './Images/Obverse/Brown/')
    templates = loadImages('grey', './Images/PatternMatchTemplate/')
    ...
    save = input("Save prediction function? (y/n) ")
    if save == "y":
        with open('LHCB_Prediction_Function.txt', 'w') as writer:
            for coefficient in LHCBp.coefficients:
                writer.write(str(coefficient) + '\n')

    obverse = loadImages('grey', './Images/Obverse/Red/')
    templates = loadImages('grey', './Images/PatternMatchTemplate/')
    ...
    save = input("Save prediction function? (y/n) ")
    if save == "y":
        with open('LHCR_Prediction_Function.txt', 'w') as writer:
...
def imgIsMSD(path):
    ...
    percentage = ImageHSV.Image_HSV(path)
    if percentage < 3:
        return True
    return False
...
def gradeCoin(path, isMSD, isBrown):
    ...
    if isMSD:
        ...
        with open('MSD_Prediction_Function.txt', 'r') as reader:
            for line in reader:
                coefficients.append(float(line))
    elif isBrown:
        templates = loadImages('grey', './Images/PatternMatchTemplate/')

        with open('LHCB_Prediction_Function.txt', 'r') as reader:
            for line in reader:
                coefficients.append(float(line))
    else:
        templates = loadImages('grey', './Images/PatternMatchTemplate/')

        with open('LHCR_Prediction_Function.txt', 'r') as reader:
            for line in reader:
                coefficients.append(float(line))
...
if __name__ == '__main__':
    ...
    dirName = os.path.abspath('starter_kit_mostly_fixed/obverse/Morgan 1880-CC MS66+ PCGS obverse.jpg')

    grade = gradeCoin(dirName, True, False)

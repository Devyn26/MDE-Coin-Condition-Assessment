...
ABS_TEST_PDF = REPO_ROOT / "MorganSilverDollar" / "Morgan_Dollar_main" / "test.pdf"
...
class App(QWidget):
    ...
    def on_grade_click(self):
        if not self._both_selected() or self.busy.isVisible():
            self._update_status_panel()
            return

        self.imagesSelected.emit(self.obverse_path, self.reverse_path)

        self.busy.setVisible(True)
        self.btn_grade.setEnabled(False)

        self.worker_thread = QThread(self)
        self.worker = GradeWorker(self.obverse_path, self.reverse_path)
        self.worker.moveToThread(self.worker_thread)

        self.worker_thread.started.connect(self.worker.run)
        self.worker.finished.connect(self._on_grade_finished)
        self.worker.failed.connect(self._on_grade_failed)
    ...
    @pyqtSlot(dict)
    def _on_grade_finished(self, data: dict):
        self.fft_results = data.get("fft", "----")
        self.fm_results = data.get("fm", "----")
        self.last_report_path = data.get("report_path", "") or ""

        self.result_box.setText(
            f"FFT Analysis Results:\n{self.fft_results}\n\n"
            f"Feature Match Results:\n{self.fm_results}\n"
            f"{'Report: ' + self.last_report_path if self.last_report_path else ''}"
        )
        self.busy.setVisible(False)
        self._update_status_panel()
...
if __name__ == '__main__':
    ...
    # Demo mode shows how the app behaves and opens a known test PDF.
    demo_mode = ('--demo' in sys.argv)

    if demo_mode:
        print("[DEMO] Demo mode enabled. Pick two images, then click 'Grade'.")

        preferred_pdf = ABS_TEST_PDF

        # When the user selects images, report the selections and open the test PDF.
        def on_images_selected(obv, rev):
            msg = f"[DEMO] imagesSelected -> obverse: {obv} | reverse: {rev}"
            print(msg)
            ui.result_box.append("\n" + msg)

            if preferred_pdf.exists():
                demo_pdf = str(preferred_pdf)
                ui.result_box.append(f"[DEMO] Using existing PDF: {demo_pdf}")
                print(f"[DEMO] Using existing PDF: {demo_pdf}")
            else:
                ui.result_box.append(f"[DEMO] Preferred PDF not found at {preferred_pdf}. Creating fallback Reports/test.pdf")
                print(f"[DEMO] Preferred PDF not found at {preferred_pdf}. Creating fallback Reports/test.pdf")
                demo_pdf = create_demo_pdf("Reports/test.pdf")

            ui.set_report_path(demo_pdf, auto_open=False)
            ui.open_report()  # explicitly open viewer for the demo

        ui.imagesSelected.connect(on_images_selected)
